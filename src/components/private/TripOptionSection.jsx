...
const TripOptionSection = ({ form, set, errors, destinationsData }) => {
  return (
    <SectionCard
      icon={
      ...
      <div className="space-y-4">
        {TRIP_OPTIONS.map((opt) => {
          ...
          return (
            <div key={opt.id}>
              <div
                className={`flex items-start justify-between gap-3 p-3.5 rounded-lg border cursor-pointer transition-colors ${
                  active
              ...
              {opt.id === "custom" && active && (
                <div className="mt-2.5 pl-1">
                  <input
                    type="text"
                    placeholder="Contoh: Trip Lansia Khusus Reuni Alumni Yogyakarta"
                  ...
                  {errors.customTripName && (
                    <p className="text-xs text-red-500 mt-1">{errors.customTripName}</p>
                  )}
                </div>
              )}
              ...
              {opt.id === "explorer" && active && (
                <div className="mt-3 pl-1">
                  {form.selectedDestinasi ? (
                    ...
                    <div>
                      <p className="text-xs font-semibold text-gray-500 mb-2.5">
                        Pilih salah satu destinasi wisata:
                      </p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-80 overflow-y-auto pr-1 rounded-lg">
                  ...
                  {errors.selectedDestinasi && (
                    <p className="text-xs text-red-500 mt-1">{errors.selectedDestinasi}</p>
                  )}
                </div>
              )}
            </div>
...
export default TripOptionSection;