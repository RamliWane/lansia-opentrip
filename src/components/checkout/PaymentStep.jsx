    ...
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-6 items-start">
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          {/* Destination row */}
          {destination && (
            <div className="flex gap-3 p-3 rounded-2xl bg-gray-50 border border-gray-100 mb-4">
            ...
            {[
              {
                label: "ID Pesanan",
                value: (
                  <span className="font-mono text-[10px] font-bold px-2 py-0.5 rounded-lg bg-orange-50 text-[#df7224]">
                    {orderId}
                  </span>
                ),
              },
              ...
              <div className="space-y-1.5">
                {participants.map((p, i) => (
                  <div key={p.id} className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold shrink-0 bg-orange-50 text-[#df7224]">
                      {i + 1}
                    </div>
          ...
          {customer.specialRequest && (
            <div className="mt-3 p-2.5 rounded-xl bg-orange-50 border border-orange-100">
              <p className="text-[10px] font-bold text-[#df7224] uppercase tracking-wider mb-0.5">
                Permintaan Khusus
              </p>
              <p className="text-xs text-[#c3611c]">{customer.specialRequest}</p>
            </div>
          )}
          ...
          <button
            onClick={onBack}
            disabled={isLoading}
            className="mt-4 flex items-center gap-1 text-xs font-semibold text-gray-400 hover:text-[#df7224] transition-colors"
          >
        ...
        {/* Error */}
        {error && (
          <div className="p-4 rounded-xl bg-red-50 border border-red-200 flex items-start gap-2.5">
