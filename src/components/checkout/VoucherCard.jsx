...
export default function VoucherCard({ voucherCode, appliedVoucher, voucherError, onCodeChange, onApply, onRemove, subtotal }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      ...
      {appliedVoucher ? (
        <div className="flex items-center justify-between p-3.5 rounded-xl border bg-orange-50 border-orange-100">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-orange-100 text-[#df7224]">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <polyline points="20 6 9 17 4 12" />
              </svg>
            </div>
            <div>
              <p className="text-xs font-bold font-mono text-[#df7224]">{appliedVoucher.code}</p>
        ...
        <>
          <div className="flex gap-2">
            <input
              ...
              className={`flex-1 px-3 py-2.5 rounded-xl text-sm font-mono font-bold text-gray-900 border focus:outline-none focus:ring-2 focus:ring-[#df7224]/20 focus:border-[#df7224] transition-colors ${voucherError ? "border-red-300" : "border-gray-200"}`}
            />
            <button
              type="button"
              onClick={onApply}
              disabled={!voucherCode.trim()}
              className="px-4 py-2.5 rounded-xl text-white text-sm font-bold bg-[#df7224] hover:bg-[#c3611c] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Pakai
            </button>
          </div>
      ...
      {!appliedVoucher && (
        <div className="mt-3">
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-[#df7224] transition-colors"
          >
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z" />
              <line x1="7" y1="7" x2="7.01" y2="7" />
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-xl border transition-all ${
                      eligible
                        ? "border-orange-100 bg-orange-50 cursor-pointer"
                        : "border-gray-100 bg-gray-50 opacity-50 cursor-not-allowed"
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${eligible ? "bg-orange-100 text-[#df7224]" : "bg-gray-100 text-gray-400"}`}>
