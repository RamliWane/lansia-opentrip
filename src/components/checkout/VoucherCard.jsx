    ...
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-sm p-5">
      ...
      {appliedVoucher ? (
        <div
          ...
          style={{ backgroundColor: "#df722408", borderColor: "#df722830" }}
        >
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: "#df722415", color: "#df7224" }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <polyline points="20 6 9 17 4 12" />
              </svg>
            </div>
            <div>
              <p className="text-xs font-extrabold font-mono" style={{ color: "#df7224" }}>{appliedVoucher.code}</p>
              <p className="text-[10px] text-gray-500">{appliedVoucher.label}</p>
            </div>
          </div>
          <div className="flex items-center gap-2.5">
            <span className="text-sm font-extrabold" style={{ color: "#df7224" }}>
            ...
            <button
              type="button"
              onClick={onRemove}
              className="w-6 h-6 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-500/10 transition-colors"
            >
        ...
        <>
          <div className="flex gap-2">
            <input
              ...
              className={`glass-input flex-1 px-3 py-2.5 rounded-xl text-sm font-mono font-bold focus:outline-none ${voucherError ? "border-red-300" : ""}`}
            />
            <button
              type="button"
              onClick={onApply}
      ...
      {!appliedVoucher && (
        <div className="mt-3">
          ...
          >
            ...
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`transition-transform duration-200 ${showList ? "rotate-180" : ""}`}>
              ...
              {AVAILABLE_VOUCHERS.map((v) => {
                ...
                return (
                  <div
                    ...
                    style={eligible
                      ? { borderColor: "#df722825", backgroundColor: "#df722808", cursor: "pointer" }
                      : { borderColor: "#f3f4f6", backgroundColor: "#f9fafb", opacity: 0.5, cursor: "not-allowed" }
                    }
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={eligible ? { backgroundColor: "#df722415", color: "#df7224" } : { backgroundColor: "#f3f4f6", color: "#9ca3af" }}>
                      ...
                      <div>
                        <p className="text-xs font-extrabold text-gray-800 dark:text-gray-200 font-mono">{v.code}</p>
                    ...
                    <div className="text-right">
                      <span className="text-xs font-extrabold" style={{ color: eligible ? "#df7224" : "#9ca3af" }}>
                        {v.type === "percentage" ? `${v.percentageValue}% off` : `−${OrderDomain.formatPrice(v.discount)}`}
                      </span>
                      {eligible && <p className="text-[10px] font-bold mt-0.5" style={{ color: "#df7224" }}>Gunakan →</p>}
                    </div>
                  </div>
                );
              })}
