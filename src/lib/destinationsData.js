export const destinationsData = [
  {
    id: 1,
    image: "https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Pantai Kuta Bali",
    location: "Bali, Indonesia",
    rating: 4.8,
    reviewCount: 3241,
    priceMin: 350000,
    priceMax: 750000,
    category: "Pantai",
    description: "Pantai Kuta adalah salah satu tempat wisata paling terkenal di Bali. Terkenal dengan garis pantai berpasir putih yang panjang, ombak yang cocok untuk berselancar pemula, dan pemandangan matahari terbenam (sunset) yang sangat memukau.",
    highlights: ["Sunset Terindah", "Aktivitas Surfing", "Dekat Pusat Belanja", "Kuliner Pantai"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 2,
    image: "https://images.unsplash.com/photo-1555400038-63f5ba517a47?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1555400038-63f5ba517a47?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Candi Borobudur",
    location: "Jawa Tengah, Indonesia",
    rating: 4.7,
    reviewCount: 5812,
    priceMin: 150000,
    priceMax: 450000,
    category: "Budaya",
    description: "Candi Borobudur adalah candi Buddha terbesar di dunia yang dibangun pada abad ke-9. Merupakan salah satu situs warisan dunia UNESCO, Borobudur terkenal dengan arsitekturnya yang megah, stupa yang indah, dan relief bersejarah yang menceritakan ajaran Buddha.",
    highlights: ["Warisan Dunia UNESCO", "Sunrise Tour Megah", "Sejarah & Edukasi", "Spot Foto Klasik"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 3,
    image: "https://images.unsplash.com/photo-1516690561799-46d8f74f9abf?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1516690561799-46d8f74f9abf?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Raja Ampat",
    location: "Papua Barat, Indonesia",
    rating: 4.9,
    reviewCount: 1987,
    priceMin: 2500000,
    priceMax: 6000000,
    category: "Pulau",
    description: "Kepulauan Raja Ampat di Papua Barat dikenal sebagai surga bawah laut dunia. Memiliki keanekaragaman hayati laut tertinggi di planet bumi, tempat ini menawarkan gugusan pulau karang eksotis dan spot diving terbaik yang sangat memanjakan mata.",
    highlights: ["Surga Diving Dunia", "Gugusan Karang Eksotis", "Fauna Langka", "Eko-Wisata Premium"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 4,
    image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Gunung Bromo",
    location: "Jawa Timur, Indonesia",
    rating: 4.6,
    reviewCount: 4103,
    priceMin: 250000,
    priceMax: 850000,
    category: "Gunung",
    description: "Gunung Bromo adalah salah satu gunung berapi aktif paling ikonik di Indonesia. Terkenal dengan lautan pasirnya yang luas, kawah yang megah, dan pemandangan matahari terbit dengan latar belakang kabut mistis di Penanjakan.",
    highlights: ["Matahari Terbit Ikonik", "Keliling Naik Jeep 4x4", "Lautan Pasir Bromo", "Pendakian Mudah"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 5,
    image: "https://images.unsplash.com/photo-1552832230-c0197dd311b5?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1552832230-c0197dd311b5?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Danau Toba",
    location: "Sumatera Utara, Indonesia",
    rating: 4.5,
    reviewCount: 2876,
    priceMin: 300000,
    priceMax: 900000,
    category: "Danau",
    description: "Danau Toba adalah danau vulkanik terbesar di dunia yang terbentuk akibat letusan supervolcano purba. Di tengah danau ini terdapat Pulau Samosir yang menyimpan pesona budaya suku Batak yang kental serta pemandangan bukit hijau yang menyejukkan.",
    highlights: ["Danau Vulkanik Terbesar", "Budaya Batak Samosir", "Udara Pegunungan Sejuk", "Resort Tepi Danau"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 6,
    image: "https://images.unsplash.com/photo-1504214208698-ea1916a2195a?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1504214208698-ea1916a2195a?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Komodo Island",
    location: "Nusa Tenggara Timur, Indonesia",
    rating: 4.8,
    reviewCount: 1543,
    priceMin: 1200000,
    priceMax: 3500000,
    category: "Pulau",
    description: "Pulau Komodo merupakan habitat asli kadal terbesar di dunia, biawak komodo. Taman Nasional ini menawarkan keindahan sabana kering yang dramatis, perbukitan yang hijau di musim hujan, dan pantai dengan air yang sangat jernih.",
    highlights: ["Melihat Komodo Langsung", "Trekking Viewpoint Ikonik", "Snorkeling Air Jernih", "Situs UNESCO"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 7,
    image: "https://images.unsplash.com/photo-1567538096630-e0c55bd6374c?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1567538096630-e0c55bd6374c?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Pantai Pink Labuan Bajo",
    location: "Nusa Tenggara Timur, Indonesia",
    rating: 4.7,
    reviewCount: 892,
    priceMin: 800000,
    priceMax: 2000000,
    category: "Pantai",
    description: "Pantai Pink adalah satu dari sedikit pantai berpasir pink di dunia. Warna pink berasal dari organisme mikroskopis Foraminifera yang bercampur dengan pasir putih. Pantai ini menawarkan gradasi air laut biru toska yang sangat menawan.",
    highlights: ["Pasir Pink Unik", "Gradasi Laut Toska", "Sempurna untuk Snorkeling", "Spot Foto Estetik"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 8,
    image: "https://images.unsplash.com/photo-1562602833-0f4ab2fc46e5?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1562602833-0f4ab2fc46e5?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Kawah Ijen",
    location: "Jawa Timur, Indonesia",
    rating: 4.6,
    reviewCount: 3217,
    priceMin: 100000,
    priceMax: 400000,
    category: "Gunung",
    description: "Kawah Ijen terkenal di seluruh dunia karena fenomena alam langka 'Blue Fire' atau api biru yang hanya bisa dilihat sebelum fajar. Kawah asam terbesar di dunia ini juga menawarkan pemandangan danau belerang berwarna hijau toska yang menakjubkan.",
    highlights: ["Fenomena Langka Blue Fire", "Danau Kawah Hijau Toska", "Trekking Menantang", "Melihat Penambang Belerang"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 9,
    image: "https://images.unsplash.com/photo-1555993539-1732b0258235?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1555993539-1732b0258235?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Pantai Nusa Penida",
    location: "Bali, Indonesia",
    rating: 4.7,
    reviewCount: 2610,
    priceMin: 450000,
    priceMax: 1100000,
    category: "Pantai",
    description: "Nusa Penida adalah pulau kecil di sebelah tenggara Bali yang terkenal dengan tebing-tebing karang raksasa. Pantai Kelingking dengan bentuk tebing mirip T-Rex dan Broken Beach merupakan ikon utama yang mendunia dari tempat ini.",
    highlights: ["Tebing Kelingking T-Rex", "Broken Beach & Angel Billabong", "Spot Diving Manta Ray", "Suasana Eksotis"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
  {
    id: 10,
    image: "https://images.unsplash.com/photo-1473625247510-8ceb1760943f?w=600&q=80",
    images: [
      "https://images.unsplash.com/photo-1473625247510-8ceb1760943f?w=600&q=80",
      "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=600&q=80",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=600&q=80"
    ],
    title: "Gili Trawangan",
    location: "Nusa Tenggara Barat, Indonesia",
    rating: 4.5,
    reviewCount: 4401,
    priceMin: 500000,
    priceMax: 1800000,
    category: "Pulau",
    description: "Gili Trawangan adalah pulau terbesar di antara tiga pulau kecil (gili) di barat laut Lombok. Pulau bebas polusi ini melarang kendaraan bermotor, menyuguhkan pantai berpasir putih, konser musik malam yang seru, dan penangkaran penyu bawah air.",
    highlights: ["Bebas Polusi Kendaraan", "Bersepeda Keliling Pulau", "Snorkeling Bersama Penyu", "Sunset Swing Romantis"],
    itinerary: [
      { day: 1, title: 'Kedatangan & Eksplorasi Awal', description: 'Tiba di lokasi, check-in, dan eksplorasi seputar area penginapan.' },
      { day: 2, title: 'Tour Utama', description: 'Menikmati destinasi utama dari pagi hingga sore.' },
      { day: 3, title: 'Waktu Bebas & Kepulangan', description: 'Belanja oleh-oleh dan persiapan pulang.' }
    ],
    meetingPoints: [
      { time: '08:00', location: 'Bandara/Stasiun Utama', description: 'Berkumpul di pintu kedatangan dengan pemandu kami.' },
      { time: '09:00', location: 'Lobi Hotel', description: 'Bagi yang sudah di lokasi, bisa langsung bertemu di lobi.' }
    ],
    reviewsList: [
      { author: 'Budi Santoso', rating: 5, date: '12 Mei 2026', comment: 'Pengalaman yang luar biasa, sangat direkomendasikan!', images: ['https://images.unsplash.com/photo-1522770179533-24471fcdba45?w=200&q=80'] },
      { author: 'Siti Aminah', rating: 4, date: '03 Juni 2026', comment: 'Pemandangannya indah, tapi perjalanan cukup melelahkan.' }
    ],
  },
];
